document.addEventListener("turbolinks:load", () => {
  ...
  if (newCard !== null) {
    newCard.addEventListener("click", (event) => {
      ...
      document.querySelector("#payment-form").classList.remove("form-none")
      document.querySelector("#existing-card").classList.add("form-none")
    })
  }
})
    ...
    if (form.dataset.status == "requires_action") {
      stripe.confirmCardPayment(paymentIntentId, {
        ...
        if (result.error) {
          ...
          form.querySelector("#card-details").classList.remove("form-none")
        } else {
          form.submit()
        }
      })
